<?php
/*
 * PHPArticle Template Management for PHP4
 *
 * (C) Copyright 2004-2006 phparticle.net
 *                    niuboy
 *
 * 2006.02.28 11:29
 */
...
class Template
{
  ...
  */
  ...
  var $htmlext	= "htm";
  var $srcext	= "import";
  var $import_src_filename = array();
  var $parent_template_filename = "";
  var $type = "T_TEMPLATE";
  var $savedir = "default";
  ...
  *
  * usage: Template([string $htmldir = "."], [string $unknowns = "remove"])
  *
  * @param     $htmldir        path to template directory
  ...
  * @return    void
  */
  function Template($htmldir = ".", $srcdir=".", $unknowns = "remove") {
    if ($this->debug & 4) {
      echo "<p><b>Template:</b> root = $htmldir, unknowns = $unknowns</p>\n";
    }
    $this->set_dir($htmldir, $srcdir);
    $this->set_unknowns($unknowns);
  }


 /******************************************************************************
  * Checks that $htmldir is a valid directory and if so sets this directory as the
  * base directory from which templates are loaded by storing the value in
  * $this->htmldir. Relative filenames are prepended with the path in $this->htmldir.
  *
  * Returns true on success, false on error.
  *
  * usage: set_dir(string $htmldir)
  *
  * @param     $htmldir         string containing new template directory
  * @see       root
  * @access    public
  * @return    boolean
  */
  function set_dir($htmldir, $srcdir) {
    if ($this->debug & 4) {
      echo "<p><b>set_dir:</b> root = $htmldir</p>\n";
    }
    if (!is_dir($htmldir) || !is_dir($srcdir)) {
      $this->halt("set_dir: $htmldir is not a directory.");
      return false;
    }
  ...
  * @return    boolean
  */
  function set_file($varname, $filename = "", $filetype="htm") {
    if (!is_array($varname)) {
      if ($this->debug & 4) {
      ...
      $filename.=".".$filetype;
      if($filetype == $this->htmlext)
        $this->file[$varname] = $this->filename($filename, $this->htmldir);
      else $this->file[$varname] = $this->filename($filename, $this->srcdir);
    } else {
      reset($varname);
      while(list($v, $f) = each($varname)) {
        ...
        $f.=".".$filetype;
        if($filetype == $this->htmlext)
          $this->file[$v] = $this->filename($f, $this->htmldir);
        else $this->file[$v] = $this->filename($f, $this->srcdir);
      }
    }
    return true;
  }
   ...
   *处理嵌套的子模板
   */
   function recursehtmlblock($str,$upname="",$htmlblockdata=Array(),$loops=0){
	$p = 0;
	while (($p = strpos($str, '<!-- BEGIN ', $p)) !== false) {
		$p += 11;
		$e = strpos($str, '-->', $p);
		$pp = strpos($str, '<!-- BEGIN ', $p);
		if($e > $pp && $pp)continue;
		$htmlname = trim(substr($str, $p, ($e - $p)));
		$rp=0;
		if(($rp = strpos($str,"<!-- END ".$htmlname." -->",$e))!==false)
		{
			if($rp>$p)
			{
				$htmldata = substr($str,$e+3,$rp-$e-3);
				if(!empty($htmldata))
				{
					$htmlblockdata[$htmlname]=$htmldata;
					if($loops>0)
					{
						$htmlblockdata[$upname]=str_replace("<!-- BEGIN ".$htmlname." -->".$htmldata."<!-- END ".$htmlname." -->","\$".$htmlname,$htmlblockdata[$upname]);
					/*	if($loops>1)
						{
							$htmlblockdata[$upname]=str_replace("$htmldata","\$".$htmlname,$htmlblockdata[$upname]);
						}else{
							$htmlblockdata[$upname]=str_replace("$htmldata","<?=\$".$htmlname."?>",$htmlblockdata[$upname]);
						}*/
					}	$this->recursehtmlblock($htmldata,$htmlname,$htmlblockdata,$loops+1);
				}
				$p=$rp+10;
			}
		}
	}
	return $htmlblockdata;
  }
/*  function recursehtmlblock($reg,$tempdata,$loops=0){
	global $tagname,$htmlblockdata,$varvals,$list_tmpl;
	preg_match_all($reg, $tempdata, $htmlblock);

	foreach($htmlblock[0] AS $key => $htmldata)
	{
		if(!empty($htmldata))
		{
			$htmlname=$htmlblock[1][$key];
			$namelen=strlen($htmlname);
			$tagname[$loops][$key]=$htmlblock[1][$key];
			$htmlblockdata[$htmlname]=$htmldata;
			if($loops>0)
			{
				$htmlblockdata[$tagname[$loops-1][$key]]=str_replace("$htmldata","\\\$".$tagname[$loops][$key],$htmlblockdata[$tagname[$loops-1][$key]]);
			}
			$this->type = "T_HTML_BLOCK";
			$list_tmpl[] = $htmlname;
			$varvals[$this->type." ".$htmlname]=$htmlblockdata[$htmlname];
			recursehtmlblock($reg,substr($htmldata,6+$namelen,-5-$namelen),$loops+1);
		}
	}
  }*/
  ...
  */
  ...
  function set_block($parent, $varname, $tempdata="", $name = "") {
    if ($this->debug & 4) {
    ...
    reset($parent);
	foreach($parent AS $importsrc)
	{
		if (!$this->loadfile($importsrc)) {
			$this->halt("set_block: unable to load $parent.");
			return false;
		}
	    $str = $this->get_var($importsrc);
	    $reg = "/[ \t]*<!--\s+BEGIN $varname\s+-->\s*?\n?(\s*.*?\n?)\s*<!--\s+END $varname\s+-->\s*?\n?/sm";
	    preg_match_all($reg, $str, $m);
	    //这里需要这样组织编写，不知道为何？不能够用$m[1][0]来判断
	    $str = $m[1][0];
		if($str)
		{
			break;
	...
	if($tempdata)
	{
	//	$reg = "#<!--\s+BEGIN\s+([^\s]+)\s+-->((?!<\!--\s+\w+\s+[^\s]+\s+-->).|(?R))*<!--\s+END\s+[^\s]+\s+-->#simU";
	//	$reg = "/[ \t]*<!--\s+BEGIN ([^\s]+)\s+-->\s*?\n?(\s*.*?\n?)\s*<!--\s+END ([^\s]+)\s+-->\s*?\n?/sm";
	//	$pregp ="#<!--\s+BEGIN\s+([^\s]+)\s+-->.*?<!--\s+END\s+[^\s]+\s+-->#si";
		$htmlblockdata=$this->recursehtmlblock($tempdata);
	/*	foreach($htmlblock AS $key => $htmlarray)
		{
			if($key%3==0)
			{
				foreach($htmlarray AS $key2 => $htmldata)
				{
					if(!empty($htmldata))
					{
						$htmlname=$htmlblock[$key+1][$key2];
						$htmlblockdata[$htmlname]=$htmldata;
						if($key>0)
						{
							$htmlblockdata[$htmlblock[$key-2][$key2]]=str_replace("$htmldata","\\\$".$htmlblock[$key-2][$key2],$htmlblockdata[$htmlblock[$key-2][$key2]]);
						}
						$this->type = "T_HTML_BLOCK";
						$list_tmpl[] = $htmlname;
						$varvals[$this->type." ".$htmlname]=$htmlblockdata[$htmlname];
		...
		foreach($htmlblock[1] AS $key => $htmlname)
		{
			$htmlblockdata[$htmlname]=$htmlblock[2][$key];
			$this->type = "T_HTML_BLOCK";
			$list_tmpl[] = $htmlname;
			$varvals[$this->type." ".$htmlname]=$htmlblockdata[$htmlname];
		}*/
    ...
    //get token
	$p = 0;
	while (($p = strpos($str, '{T_', $p)) !== false) {
		$p += 1;
		$e = strpos($str, '/}', $p);
		$pp = strpos($str, '{T_', $p);
		if($e > $pp && $pp)continue;
		$temp_tmpl = substr($str, $p, ($e - $p));
		$target_tmpl = explode(' ', $temp_tmpl);//echo $temp_tmpl."aaa\n\n\n\n\n";
		if($target_tmpl[0] == 'T_IMPORT_SRC'){
			$this->set_file($target_tmpl[1], $target_tmpl[1], $this->srcext);
			$this->import_src_filename[$target_tmpl[1]] = $target_tmpl[1];
			$import_name[] = $target_tmpl[1];
		}elseif($target_tmpl[0] == 'T_TEMPLATE'){//echo $target_tmpl[1]."<br>";
			$this->type = "T_TEMPLATE";
			$list_tmpl[] = $target_tmpl[1];
			$this->set_file($target_tmpl[1], $target_tmpl[1]);
			$varvals[$this->type." ".$target_tmpl[1]]=$this->parse($target_tmpl[1], $target_tmpl[1]);
		}elseif($target_tmpl[0] == 'T_CODE_BLOCK'){
			$this->type = "T_CODE_BLOCK";
			$list_tmpl[] = $target_tmpl[1];
			$varvals[$this->type." ".$target_tmpl[1]]=$this->set_block($this->import_src_filename, $target_tmpl[1]);
		}elseif($target_tmpl[0] == 'T_HTML_BLOCK'){
			$this->type = "T_HTML_BLOCK";
			$list_tmpl[] = $target_tmpl[1];
			$varvals[$this->type." ".$target_tmpl[1]]=$htmlblockdata[$target_tmpl[1]];
		}elseif($target_tmpl[0] == 'T_HTML_CODE'){
			$this->type = "T_HTML_CODE";
			$list_tmpl[] = $target_tmpl[1];
			$varvals[$this->type." ".$target_tmpl[1]]=$htmlblockdata[$target_tmpl[1]];
		}else{$p = $e + 1;continue;}
		$p = $e + 1;
	...
	if($import_name)
	foreach($import_name AS $src_name)
	{
		unset($this->import_src_filename[$src_name]);
	}
	$str = $this->replace_token($str, $varvals);
    return $str;
  }
  ...
  */
  ...
  //  foreach($varvals_quoted AS $key => $detail)if($key !="tpl_threadlist")echo $key."=>".$this->varkeys[$key]."-----".$detail."\n\n\n";
//echo $str."\n\n\n\n\n";
    $str = preg_replace($this->varkeys, $varvals_quoted, $str);
    return $str;
  }

function replace_token($str,$varvals) {
if(is_array($varvals)){
    reset($varvals);
    while(list($k, $v) = each($varvals)) {//echo $v."<br><br><br><br><br>".$str;
      $varvals_quoted[$k] = preg_replace(array('/\\\\/', '/\$/'), array('\\\\\\\\', '\\\\$'), $v);
      $varkeys[$k]="/\{".$k."\s*\/\}/sU";//需要加s来处理多行字符窜
    }
    //foreach($varkeys AS $key => $detail){echo $detail."<br>";}
    $str = preg_replace($varkeys, $varvals_quoted, $str);
//    echo "<br><br><br><br><br>".$str;exit;
}
    return $str;
  }
  ...
  */
  ...
  function parse($target, $varname, $append = false) {
	...
	while (($p = strpos($str, '{T_', $p)) !== false) {
		$p += 1;
		$e = strpos($str, '/}', $p);
		if(!$e)$this->halt("Can not find symbol '/}',process exit!");
		$temp_tmpl = substr($str, $p, ($e - $p));
		$target_tmpl = explode(' ', $temp_tmpl);
		if($target_tmpl[0] == 'T_IMPORT_SRC'){
			$this->set_file($target_tmpl[1], $target_tmpl[1], $this->srcext);
			$this->import_src_filename[$target_tmpl[1]] = $target_tmpl[1];
			$varvals["T_IMPORT_SRC ".$target_tmpl[1]] = '';
			$import_name[] = $target_tmpl[1];
		}
		if($target_tmpl[0] == 'T_TEMPLATE'){
			$this->type = "T_TEMPLATE";
			$list_tmpl[] = $target_tmpl[1];
			$this->set_file($target_tmpl[1], $target_tmpl[1]);
			$varvals[$this->type." ".$target_tmpl[1]]=$this->parse($target_tmpl[1], $target_tmpl[1]);
			...
			$list_tmpl[] = $target_tmpl[1];
			$varvals[$this->type." ".$target_tmpl[1]]=$this->set_block($this->import_src_filename, $target_tmpl[1]);
		}elseif($target_tmpl[0] == 'T_CHTML_BLOCK'){
			$this->type = "T_CHTML_BLOCK";
			preg_match("/^([^\s]+).*/",$target_tmpl[1],$tmpstr);
			$target_tmpl[1]=$tmpstr[1];
			$list_tmpl[]=$tmpstr[1];
			$varvals[$this->type." ".$target_tmpl[1].".*"]=$this->set_block($this->import_src_filename,$target_tmpl[1],$temp_tmpl);
		}else{$p = $e + 1;continue;}
		$p = $e + 1;
	}
    }
  ...
  */
  ...
  function save($varname) {
    $this->writetofile($this->savedir."/".$this->parent_template_filename.".php",trim($this->get_var($varname)));
  }
  ...
  * with $this->htmldir prepended. Absolute pathnames are returned unchanged.
  *
  ...
  * @see       set_dir
  */
  function filename($filename, $filedir) {
    if ($this->debug & 4) {
...
if(preg_match("/".$this->htmlext."$/",$filename)){
    $str = str_replace("\"","\\\"",implode("", @file($filename)));
}else $str = implode("", @file($filename));
    if (empty($str)) {
...
if(preg_match("/".$this->htmlext."$/",$filename)){
        eval("\$str = \"".$str."\";");
/*	$common_templates = Array('html_doctype', 'header', 'css', 'top_logo', 'is_not_login', 'my_info', 'guest_welcome_message', 'footer', 'thread_icons', 'editor', 'navigate_bar', 'navigate_buttons_bar', 'jump_to_forum', 'forum_jump', 'usercp_navigate', 'thread_rating');
	if(!in_array($varname, $common_templates)){
		echo "'".$varname."', ";
//		$ext = get_ext_name($filename);
//		if($ext == $this->htmlext && $file != 'index')
			{
				include_once("addtemplates.php");
//			$pos = strpos($filename, '.');
			$tpname = $varname;//substr($filename, 0, $pos);
			$filedata = readfromfile($filename);
			$template_un = addslashes($filedata);
			$template = preg_replace('/\"/', '\\\"', $template_un);
		}
		if(!empty($template)){
			global $DB_site;
			$DB_site->query("INSERT INTO " . TABLE_PREFIX . "template (`styleid`, `title`, `template`, `template_un`, `templatetype`, `dateline`, `username`, `templategroup`, `version`) VALUES ('-1', '$tpname', '$template', '$template_un', 'template', '".time()."', 'niuboy', '".$_GET['mod']."', 'Alpha 1.0')");
		}
