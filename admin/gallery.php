...
if ($_POST['action'] == "doseldelete") {
        ...
        if (is_array($images)) {
                foreach($images as $key => $value) {
                        $image = $DB->fetch_one_array("SELECT * FROM " . $db_prefix . "gallery WHERE id='$value'");
                        unlink("../upload/images/$image[filename]");
                        $DB->query("DELETE FROM " . $db_prefix . "gallery WHERE id='$value'");
                } 
        } 
...
if ($_POST['action'] == 'doupload') {
        ...
        for ($key = 0;$key < 10;$key++) {
                $original = $_FILES['image']['name'][$key];
			
				 //$original=$img['name'][$key];
                $filename = md5(uniqid(microtime(), 1)); 
                //echo $_FILE['image']['tmp_name'][$key];
                if (!empty($original)) {
					echo "ok";
                        $result[$key] = "<tr class=" . getrowbg() . ">
                              <td>$original</td>
                              <td>" . get_real_size($_FILES['image']['size'][$key]) . "</td>
                              <td>" . $_FILES['image']['type'][$key] . "</td>
                              <td>";
                        if ($_FILES['image']['type'][$key] == "image/jpeg" OR $_FILES['image']['type'][$key] == "image/gif" OR $_FILES['image']['type'][$key] == "image/x-png") {
							echo "ok1";
                                // copy($_FILES['image']['tmp_name'][$key], "../upload/images/$filename")
                                if (is_uploaded_file($_FILES['image']['tmp_name'][$key]) AND move_uploaded_file($_FILES['image']['tmp_name'][$key], "../upload/images/$filename")) {
                                        $DB->query("INSERT INTO " . $db_prefix . "gallery (original,filename,type,size,dateline,userid)
	                                        VALUES ('" . addslashes(trim($original)) . "','$filename','" . addslashes($_FILES['image']['type'][$key]) . "','" . addslashes($_FILES['image']['size'][$key]) . "','" . time() . "','$pauserinfo[userid]')");
        ...
        echo "<p align=center>[<a href=\"gallery.php\">查看已上传的图片</a>] [<a href=\"gallery.php?action=upload\">继续上传图片</a>] </p>";
} 
