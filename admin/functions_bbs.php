...
function dooutput($text) {
        global $debug, $showqueries, $DB_bbs, $templatecache, $gzipoutput, $gziplevel, $writename, $writedir, $pauserinfo, $phparticleurl,$g_o_back2root;
        if ($debug != 1) {
                if ($showqueries == 1) {
            ...
            //    print_rr(getallheaders());
                ...
                if ($showqueries == 1) {
                        ...
                        $script_endtime = $end_time[0] + $end_time[1];
                        $totaltime = $script_endtime - $script_starttime;
                        $stat = "<center><font face=\"verdana, arial, helvetica ,宋体\" style=\"font-size=7pt\">Page created in $totaltime seconds with $DB_bbs->querycount queries.<br>\nspending " . (round($querytime / $totaltime, 4) * 100) . "% doing MySQL queries and " . (round(($totaltime - $querytime) / $totaltime, 4) * 100) . "% doing PHP things.</font>\n</center>";
                        echo $stat;
                } 
        } 
        if (CREATE_HTML_FILE != 1)
...
function get_sortdirs($sortid = '-1') {
        global $DB_bbs, $db_prefix,$parentsort,$db_prefix_bbs,$bbsnoupforumid,$usename,$singledir,$subsort;
//	$sorts = $DB_bbs->fetch_one_array("SELECT parentlist FROM " . $db_prefix_bbs . "sort WHERE sortid='$sortid'");
//	$sortbits = explode(',', $sorts['parentlist']);
	...
	}else if($singledir==1)
	{
		...
		$sortbits[]=$sortid;
	        foreach($parentsort[$sortid] AS $parentid => $sorttitle)
	        {
	        	$sortbits[]=$parentid;
	        	$sortid=$parentid;
	        	if($sortid==$bbsnoupforumid)break;
	        }
	        foreach($sortbits AS $sortid) {
	                //if ($sortid != $bbsnoupforumid)
	                {
	                        if ($sortdirs){
...
function get_sortsubdirs($sortid = '-1', $rootdir = '-1') {
        global $DB_bbs, $db_prefix,$parentsort,$db_prefix_bbs,$bbsnoupforumid,$usename,$singledir,$subsort;
//	$sorts = $DB_bbs->fetch_one_array("SELECT parentlist FROM " . $db_prefix_bbs . "sort WHERE sortid='$sortid'");
//	$sortbits = explode(',', $sorts['parentlist']);
        if($singledir==2)
	...
	}else if($singledir==1)
	{
		...
		$sortbits[]=$sortid;
	        ...
	        foreach($sortbits AS $sortid){
	                if ($sortid != $bbsnoupforumid && $sortid != $rootdir) {
	                        if ($sortdirs)
	                                $sortdirs = ($usename?$subsort["dirname_$sortid"]:$sortid) . "/" . $sortdirs;
	                        else $sortdirs = $usename?$subsort["dirname_$sortid"]:$sortid;
	                }
...
function get_sortdepths($sortid = '-1') {
        global $DB_bbs, $db_prefix,$parentsort,$db_prefix_bbs,$bbsnoupforumid;
//	$sorts = $DB_bbs->fetch_one_array("SELECT parentlist FROM " . $db_prefix_bbs . "sort WHERE sortid='$sortid'");
//	$sortbits = explode(',', $sorts['parentlist']);
        $sortbits[]=$sortid;
        foreach($parentsort[$sortid] AS $parentid => $sorttitle)
...
unset($style);
function getstyle() {
        global $DB, $db_prefix, $configuration, $templatesetid, $styleid;
...
function validate_articleid($articleid) {
        global $DB_bbs, $db_prefix,$forumlist,$tablelist,$aconvertlist,$sconvertlist,$atconvertlist,$db_prefix_bbs;
        $articleid = intval($articleid);
        if (empty($articleid)) {
                show_errormessage("error_invalid_articleid");
        } else {
                $articleinfo = $DB_bbs->fetch_one_array("SELECT ".$aconvertlist." FROM " . $db_prefix_bbs . $forumlist['article']."
...
function validate_sortid($sortid) {
        ...
        if (empty($sortid)) {
                ...
                $sortinfo = $DB_bbs->fetch_one_array("SELECT ".$sconvertlist." FROM " . $db_prefix_bbs . $forumlist['sort']." WHERE sortid='$sortid'");
                if (empty($sortinfo)) {
                        show_errormessage("error_invalid_sortid");
                } 
                return $sortinfo;
...
function subsorts($sortid) {
        global $DB_bbs, $db_prefix, $sorts,$forumlist,$tablelist,$aconvertlist,$sconvertlist,$atconvertlist,$db_prefix_bbs,$dbname_bbs;
        $DB_bbs->selectdb2($dbname_bbs);
        $sorts = $DB_bbs->query("SELECT ".$tablelist['sort']['sortid']." AS sortid FROM " . $db_prefix_bbs . $forumlist['sort']." WHERE ".$tablelist['sort']['parentid']."='$sortid'");
        if ($DB_bbs->num_rows($sorts) > 0) {
                return true;
        } else {
                return false;
        } 
...
function getsubsorts($sortid) {
        global $DB_bbs, $db_prefix, $subsort; 
        // $sorts = $DB_bbs->query("SELECT sortid FROM ".$db_prefix."sort WHERE parentid='$sortid'");
...
function getsubsorts_first($sortid) {
        global $DB_bbs, $db_prefix, $subsort,$forumlist,$tablelist,$aconvertlist,$sconvertlist,$atconvertlist,$db_prefix_bbs;
        $sorts = $DB_bbs->query("SELECT ".$tablelist['sort']['sortid']." AS sortid FROM " . $db_prefix_bbs . $forumlist['sort']." WHERE ".$tablelist['sort']['parentid']."='$sortid'");
...
function cachesorts() {
        global $DB,$DB_bbs, $db_prefix, $subsort, $parentsort,$forumlist,$tablelist,$aconvertlist,$sconvertlist,$atconvertlist,$db_prefix_bbs,$dbname_bbs,$loadsystem_suffix;

        $caches = $DB->query("SELECT * FROM " . $db_prefix . "cache_bbs
                                        WHERE name IN ('subsort".$loadsystem_suffix."','parentsort".$loadsystem_suffix."')");
        while ($cache = $DB->fetch_array($caches)) {
        	$nonsufname = str_replace($loadsystem_suffix,"",$cache['name']);
                $$nonsufname = unserialize($cache['content']);
        ...
        // exit;
        $DB_bbs->selectdb2($dbname_bbs);
        if (!$c['subsort'.$loadsystem_suffix] OR !$c['parentsort'.$loadsystem_suffix] OR $expiry['subsort'.$loadsystem_suffix] == 1 OR $expiry['parentsort'.$loadsystem_suffix] == 1) {
                $sorts = $DB_bbs->query("SELECT ".$sconvertlist." FROM " . $db_prefix_bbs . $forumlist['sort']." ORDER BY displayorder,binary ".$tablelist['sort']['title'].",".$tablelist['sort']['sortid']);
                unset($subsortids);
                $pgjs = "paginationMax=new Array();";
                while ($sort = $DB_bbs->fetch_array($sorts)) {
                        ...
                        $total = $DB_bbs->fetch_one_array("SELECT COUNT(*) AS count FROM " . $db_prefix_bbs . $forumlist['article']." AS article
					                  WHERE ".$tablelist['article']['sortid']." IN (0" . $subsortids . ")");// AND visible=1
                        $subsort["total_$sort[sortid]"] = ($total['count'] < 1)?1:$total['count'];
                        $subsort["count_$sort[sortid]"] = $sort['articlecount'];
                        $subsort["perpage_$sort[sortid]"] = 20;//$sort['perpage'];
                        ...
                        $totalpages = ceil($sort['articlecount'] / 20);
                        $pgjs .= "paginationMax[".$sort['sortid']."]=$totalpages;";
                } 
                // print_rr($parentsort);
                $DB->selectdb();
                if ($c['subsort'.$loadsystem_suffix]) {
                        if ($expiry['subsort'.$loadsystem_suffix]) {
                                $DB->query("UPDATE " . $db_prefix . "cache_bbs SET
                                        content='" . addslashes(serialize($subsort)) . "',
                                        expiry=0
                                        WHERE name='subsort".$loadsystem_suffix."'");
                        } 
                } else {
                        $DB->query("INSERT INTO " . $db_prefix . "cache_bbs (name,content,expiry) VALUES
                                    ('subsort".$loadsystem_suffix."','" . addslashes(serialize($subsort)) . "',0)");
                } 
                if ($c['parentsort'.$loadsystem_suffix]) {
                        if ($expiry[parentsort]) {
                                $DB->query("UPDATE " . $db_prefix . "cache_bbs SET
                                        content='" . addslashes(serialize($parentsort)) . "',
                                        expiry=0
                                        WHERE name='parentsort".$loadsystem_suffix."'");
                        } 
                } else {
                        $DB->query("INSERT INTO " . $db_prefix . "cache_bbs (name,content,expiry) VALUES
                                    ('parentsort".$loadsystem_suffix."','" . addslashes(serialize($parentsort)) . "',0)");
                }
                $fp = fopen("admin/configs/pg".$loadsystem_suffix.".js",w);
                fwrite($fp,$pgjs);
		fclose($fp);
        }
}
...
function makehot_recommend_articlelist000000() {
                        ...
                        if ($sortinfo['hotarticlenum'] > 0) {
	                        ...
	                        $DB->selectdb();
                                while ($article = $DB_bbs->fetch_array($articles)) {
                                        $childdirs = preg_replace("@" . $sortdir . "/@", "", mkdirname($article['sortid'],-1,$article['date'],0,0));//get_sortdirs($article['sortid'])."/"
                       ...
                       /* unset($poparticlelist);
                        ...
                        if ($sortinfo['ratearticlenum'] > 0) {
                                ...
                                while ($article = $DB_bbs->fetch_array($articles)) {
                                        ...
                                        if (empty($childdirs))$childdirs = "./";
                                        $articlehtmllink = $childdirs . rawurlencode(mkfilename($filenamemethod,$article['title'],1)) . $article['articleid'] . "_1." . HTMLEXT;//date("Y_m", $article['date']) . "/".
                                        eval("\$poparticlelistbit .= \"" . gettemplate('popsortarticlelistbit') . "\";");
                                } 
                                $DB_bbs->free_result($articles);
       		...
       		if($contenttype!='new')
        		{
        			...
        			{
        				$selopt=$tablelist['article']['sortid']."=".$sortid;
        				$orderopt="date DESC";
        			}
        			else
        			{
        				$selopt=$tablelist['article']['sortid']." IN (0" . $subsortids . ")";
        				$orderopt="views DESC";
        			}
              
			        $articles = $DB_bbs->query("SELECT ".$aconvertlist." FROM " . $db_prefix_bbs . $forumlist['article']." 
			                             WHERE ".$selopt."
			                            ORDER BY ".$orderopt."
			                            LIMIT $maxarticles");// AND visible=1
						}else
	          {
	            $subsortids = getsubsorts_first($sortid);
			        $articles = $DB_bbs->query("SELECT ".$aconvertlist." FROM " . $db_prefix_bbs . $forumlist['article']." 
...
function gethotarticles() {
        global $DB_bbs, $db_prefix, $phparticleurl,$g_o_back2root, $hotarticlenum;
        global $style,$filenamemethod,$forumlist,$tablelist,$aconvertlist,$sconvertlist,$atconvertlist,$db_prefix_bbs,$dbname_bbs;
        $DB_bbs->selectdb2($dbname_bbs);
        $articles = $DB_bbs->query("SELECT ".$aconvertlist." FROM " . $db_prefix_bbs . $forumlist['article']." AS article
                                           LEFT JOIN " . $db_prefix_bbs . $forumlist['sort']." AS sort
                                             ON article.sortid=sort.".$tablelist['sort']['sortid']."
                                           WHERE 1
                                           ORDER BY views DESC LIMIT $hotarticlenum");// AND article.visible=1
        while ($article = $DB_bbs->fetch_array($articles)) {
                $articlehtmllink = HTMLDIR . "/" . mkdirname($article['sortid'],-1,$article['date'],0,0).rawurlencode(mkfilename($filenamemethod,$article['title'],1)) . $article['articleid'] . "_1." . HTMLEXT;//get_sortdirs($article['sortid']) . "/" . date("Y_m", $article['date']) . "/"
...
function gethotsort_articles($sortid = 0) {
        ...
        $articles = $DB_bbs->query("SELECT ".$aconvertlist." FROM " . $db_prefix_bbs . $forumlist['article']." AS article
                                           LEFT JOIN " . $db_prefix_bbs . $forumlist['sort']." AS sort
                                             ON article.sortid=sort.".$tablelist['sort']['sortid']."
                                           WHERE article.sortid=$sortid
                                           ORDER BY views DESC LIMIT $hotarticlenum");// AND sort.showinhot=1 AND article.visible=1
        while ($article = $DB_bbs->fetch_array($articles)) {
...
function getnewarticles() {
        global $DB_bbs, $db_prefix, $phparticleurl,$g_o_back2root, $lastupdatenum;
        global $style,$g_o_back2root,$filenamemethod,$forumlist,$tablelist,$aconvertlist,$sconvertlist,$atconvertlist,$db_prefix_bbs,$dbname_bbs;
        $DB_bbs->selectdb2($dbname_bbs);
        $articles = $DB_bbs->query("SELECT ".$aconvertlist." FROM " . $db_prefix_bbs . "article AS article
                                           LEFT JOIN " . $db_prefix_bbs . $forumlist['sort']." AS sort
                                             ON article.sortid=sort.".$tablelist['sort']['sortid']."
                                           WHERE 1
                                           ORDER BY date DESC LIMIT $lastupdatenum");//sort.showinlast=1 AND article.visible=1
        while ($article = $DB_bbs->fetch_array($articles)) {
                $articlehtmllink = HTMLDIR . "/" . mkdirname($article['sortid'],-1,$article['date'],0,0).rawurlencode(mkfilename($filenamemethod,$article['title'],1)) . $article['articleid'] . "_1." . HTMLEXT;//get_sortdirs($article['sortid']) . "/" . date("Y_m", $article['date']) . "/"
...
function getpoparticles() {
        global $DB_bbs, $db_prefix, $phparticleurl,$g_o_back2root, $ratearticlenum;
        global $style,$filenamemethod,$db_prefix_bbs,$dbname_bbs;
        $DB_bbs->selectdb2($dbname_bbs);
        $articles = $DB_bbs->query("SELECT articleid,article.sortid,article.date,article.title,views,(totalscore/voters) as averagescore,voters FROM " . $db_prefix_bbs . "article AS article
                                           LEFT JOIN " . $db_prefix_bbs . "sort AS sort
                                             ON article.sortid=sort.sortid
...
function gethotsorts() {
         ...
         global $DB_bbs,$db_prefix,$hotsortnum;
         $sorts = $DB_bbs->query("SELECT sortid,title,articlecount FROM ".$db_prefix."sort
...
function sortsbit_js($sortid = "-1", $level = 1) {
        global $DB_bbs, $db_prefix, $subsort, $selected;
        if (isset($subsort[intval($sortid)])) {
                foreach($subsort[intval($sortid)] as $sort['sortid'] => $sort['title']) {
...
function getparentsorts($sortid) {
        global $DB_bbs, $db_prefix,$forumlist,$tablelist,$aconvertlist,$sconvertlist,$atconvertlist,$db_prefix_bbs,$dbname_bbs;
        $DB_bbs->selectdb2($dbname_bbs);
        $sorts = $DB_bbs->query("SELECT ".$tablelist['sort']['parentid']." AS parentid,".$tablelist['sort']['title']." AS title FROM " . $db_prefix_bbs . $forumlist['sort']." WHERE sortid='$sortid'");
        while ($sort = $DB_bbs->fetch_array($sorts)) {
                $sortid .= ",";
...
function showarticle($sortid, $templatename, $main_article , $main_len) {
        global $DB_bbs, $db_prefix, $phparticleurl,$g_o_back2root;
        global $style,$filenamemethod,$forumlist,$tablelist,$aconvertlist,$sconvertlist,$atconvertlist,$db_prefix_bbs,$dbname_bbs;
        $DB_bbs->selectdb2($dbname_bbs);
        $subsortids = getsubsorts_first($sortid);
        $articles = $DB_bbs->query("SELECT ".$aconvertlist." FROM " . $db_prefix_bbs . $forumlist['article']." 
                             WHERE sortid IN (0" . $subsortids . ")
                            ORDER BY date DESC
                            LIMIT $main_article");// AND visible=1
...
function showimgarticle($sortid, $templatename, $img_article, $img_article_len) {
        global $DB_bbs, $db_prefix, $phparticleurl,$g_o_back2root;
        global $style,$filenamemethod,$dbname_bbs;
        $subsortids = getsubsorts_first($sortid);
        if ($sortid == 0) {
                $condition = "";
        } else {
	...
	$DB_bbs->selectdb2($dbname_bbs);
        $articles = $DB_bbs->query("SELECT articleid,title,description,date,imageid,sortid,author,source,contact FROM 
